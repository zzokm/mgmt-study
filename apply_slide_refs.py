...
Writes SLIDE_REF_VALIDATION.md for human review.
Does NOT modify answered-pools/.
"""
...
REPORT_PATH = ROOT / "SLIDE_REF_VALIDATION.md"
...
def write_report(rows: list[dict], errors: list[dict], manifest: dict) -> None:
    ...
    w("```")
    w("")
    w("Slide numbers map 1:1 to PDF page index in `Lectures/` (1-based).")
    w("")
    w("## Lecture manifest")
    w("")
...
def main() -> None:
    ...
    MANIFEST = ROOT / "lectures_manifest.json"
