"""Build per-lecture question pools and exam analysis from final*.json exams."""
import json
import re
...
def write_pools(by_topic: dict[str, list[dict]]) -> list[dict]:
    out_dir = Path("question-pools-by-lecture")
...
def write_repetitive_questions(all_q: list[dict]) -> dict:
    ...
    Path("repetitive-questions.json").write_text(
...
def main() -> None:
    ...
    Path("question-pools-by-lecture/_index.json").write_text(
        json.dumps(index, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    analysis = analyze(all_q, by_topic, meta)
    Path("EXAM_QUESTION_ANALYSIS.md").write_text(analysis, encoding="utf-8")
    print(
