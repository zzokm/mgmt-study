...
def main() -> None:
    ...
    (ROOT / "book_manifest.json").write_text(
        json.dumps(book_manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )

    total = 0
    for pool in (ROOT / "question-pools-by-lecture").glob("chapter-*.json"):
        n = process_file(pool, manifest, book_manifest)
        print(f"{pool.name}: {n}")
        total += n

    for name in ("final19.json", "final21.json", "final24.json", "final25.json"):
        p = ROOT / name
        if p.is_file():
    ...
    rep = ROOT / "repetitive-questions.json"
    if rep.is_file():
    ...
    for answered in (ROOT / "answered-pools").glob("chapter-*.json"):
        n = process_file(answered, manifest, book_manifest)
        print(f"answered-pools/{answered.name}: {n}")
        total += n
